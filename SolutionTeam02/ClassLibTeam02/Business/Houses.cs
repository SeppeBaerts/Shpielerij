...
namespace ClassLibTeam02.Business
{
    ...
    public static class Houses
    {
        ...
        public static IEnumerable<House> GetHouses()
        {
            ...
            foreach(DataRow row in dataTable.Rows)
            {
                object[] itemArray = row.ItemArray;
                                         //HouseiD              Street              houseNumber                areaCode        creationYear           houseType         amountRooms             floors                  floor                                                 houseSurface         gardenSurface                                                Price
        ...
        {
            InsertResult result = AddCity(house.HouseAdress.City, house.HouseAdress.AreaCode);
            if (result.Succeeded && result.Errors.Count() == 0)
                result = AddHouse(house);
            return result;
        }
